...
Pruebas unitarias y de integración con Jest: las rutas HTTP se ejercitan con supertest sobre un mock del cliente Prisma, la sesión es un JWT real firmado con el secreto de prueba (por lo que la autenticación se prueba de verdad), y el entorno se sustituye en la frontera por un mock de SSH. La terminal WebSocket se prueba levantando un servidor HTTP efímero con un cliente `ws` real, mockeando la apertura de la PTY y la base de datos. Comando: `npx jest tests/preferences tests/terminal` desde `backend/`.
...
Total de la iteración: 34 pruebas en verde, repartidas en 6 suites (7 preferencias, 4 terminal HTTP, 8 terminal-entorno, 3 heartbeat, 4 wsAuth, 8 gateway WebSocket). La suite completa del proyecto queda en 66 pruebas verdes, sumando las 21 de la iteración 1 y las 11 que la iteración 3 aporta del avance del temario y la gestión de docentes.
