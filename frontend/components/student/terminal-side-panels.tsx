...
export function TerminalSidePanels() {
  ...
  if (selectedIndex !== null) {
    ...
    const isLast = selectedIndex >= activities.length - 1
    ...
    return (
      <div className="rounded-xl border border-black/15 bg-background p-4 dark:border-border">
        ...
        <div className="mt-4 flex justify-end">
          <button
            type="button"
            onClick={() => setSelectedIndex(isLast ? null : selectedIndex + 1)}
            className="rounded-md bg-amber-500 px-3.5 py-1.5 text-sm font-medium text-white transition-colors hover:bg-amber-400"
          >
            Siguiente
          </button>
        </div>
  ...
  return (
    <>
      ...
      <div className="rounded-xl border border-black/15 bg-background p-4 dark:border-border">
        <h2 className="mb-3 text-sm font-semibold text-foreground">Actividades sugeridas</h2>
        {activities.length === 0 ? (
          ...
          <div className="space-y-2">
            {activities.map((activity, i) => (
              <button
                ...
                className="group block w-full rounded-lg border border-border p-2.5 text-left transition-colors hover:border-amber-500/40 hover:bg-amber-500/5"
              >
                <p className="text-sm font-medium text-foreground transition-colors group-hover:text-amber-500">
                  {activity.title}
                </p>
              </button>
            ))}
