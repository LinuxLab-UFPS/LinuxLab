  ...
  useEffect(() => {
    ...
    if (!oobCode) {
      queueMicrotask(() => {
        if (!cancelled) {
          setStatus("error")
          setMessage("Enlace inválido o expirado. Solicita uno nuevo desde Iniciar sesión.")
        }
      })
        ...
        {status === "loading" ? (
          ...
          <>
            ...
            <div className="mt-8 flex flex-col gap-3">
              ...
              <Button variant="outline" onClick={() => router.replace("/auth/verificacion")} className="h-11 w-full">Reenviar verificación</Button>
            </div>
          </>
        )}
      </div>
